...
cols <- c(rep('#707070', length(sitesList)), 'black') 
...
boxplot(data.table(perSiteModel$DT$abs, genModel$DT$abs), outline = F, ylim = c(0.1,.7), col = NULL, border = cols)
...
text(labels = siteNames, x = 1:c(length(sitesList)+1), y = .89, srt = 65, xpd = NA, adj=1, font=2, col = cols)
dev.off()
