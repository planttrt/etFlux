...
png('figure/etFlux.Fig.Fluxes.Box.png', width = 6, height = 4, units = 'in', res = 300)
par(mar=c(0,0,0,0), oma=c(4,4,1,1))
boxplot(abs(sapply(genModel$Samples[c("H", "Snet", "THi", "THo")], rowMeans)), outline=F, horizontal=T, col='#bbbbbb', names=NA)
#c('Sensible heat (H)','Net solar (Snet)','Incoming thermal (THi)','Outgoing thermal (THo)'
mtext(text =  c("H", "R", "LI", "LO"),
      side = 1, at = -70, xpd=T, line = -2.7-seq(0,10.5, length.out = 4), adj=0, font=2, cex=1.7)
mtext('Energy flux (W/m²)', font = 2, cex=1.4, line = 2.5, side = 1)
dev.off()
