...
par(mar=c(1,1,1,1), mfrow=c(3,1), oma=c(2,2,2,1), bty='n')
plot(r1, col = col, legend = F, xaxt='n')
map('usa', add = T)
plot(physio, add=T)
mtext('Sensitivity to ∆T, Jul-Aug-Sept', font=2, line = 0, cex=1)
insertLegend(quantile(r1, probs=c(.01,.99)), col)

plot(r2, col = col, legend = F, xaxt='n')
map('usa', add = T)
plot(physio, add=T)
mtext('Sensitivity to TS, Jul-Aug-Sept', font=2, line = 0, cex=1)
insertLegend(quantile(r1, probs=c(.01,.99)), col)

plot(r3, col = col, legend = F)
map('usa', add = T)
plot(physio, add=T)
mtext('Sensitivity to TA, Jul-Aug-Sept', font=2, line = 0, cex=1)
insertLegend(quantile(r1, probs=c(.01,.99)), col)
